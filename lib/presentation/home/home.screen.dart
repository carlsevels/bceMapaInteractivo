...
class HomeScreen extends GetView<HomeController> {
            ...
            () => AnimatedPadding(
              ...
              child: Stack(
                children: [
                  ...
                  _buildZoomControls(),
                  _buildFloatingMenuButton(), // Botón con restricción de tutorial
                  _buildMissionBanner(),
                ],
              ),
            ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // FOCO VISUAL PARA EL PASO 2 (Buscador)
          Obx(
            () => _tutorialHighlight(
          ...
          // FOCO VISUAL PARA EL PASO 1 (Pisos)
          Obx(
            () => _tutorialHighlight(
  ...
  Widget _buildZoomControls() {
    return Positioned(
      ...
      child: Column(
        children: [
          _miniFab(Icons.add, () => controller.zoomIn()),
          const SizedBox(height: 10),
          _miniFab(Icons.fullscreen_exit, () => controller.resetZoom()),
          const SizedBox(height: 10),
          _miniFab(Icons.remove, () => controller.zoomOut()),
        ],
      ),
    );
  }
  ...
  Widget _miniFab(IconData i, VoidCallback call) => FloatingActionButton(
    ...
    backgroundColor: Colors.white,
    onPressed: call,
    child: Icon(i, color: Colors.cyan[800]),
  );
  ...
  Widget _buildFloatingMenuButton() {
    return Positioned(
      ...
      left: 30,
      child: Obx(() {
        // Si el tutorial está activo (pasos 1 a 3), el botón se bloquea visualmente
        bool tutorialActivo = controller.missionStep.value > 0 && controller.missionStep.value < 4;
        
        return Opacity(
          opacity: tutorialActivo ? 0.5 : 1.0, // Se ve más tenue si está bloqueado
          child: FloatingActionButton.extended(
            heroTag: 'menu_fab',
            backgroundColor: Colors.white,
            onPressed: tutorialActivo 
                ? null // Deshabilita el click durante el tutorial
                : () => controller.isMenuOpen.toggle(),
            label: Text(
              controller.isMenuOpen.value ? "CERRAR" : "MENÚ",
              style: TextStyle(color: tutorialActivo ? Colors.grey : Colors.cyan),
            ),
            icon: Icon(
              Icons.menu, 
