...
class HomeScreen extends GetView<HomeController> {
      ...
      child: Column(
        ...
        children: [
          const Text('BIBLIOTECA\nINTERACTIVA', 
            style: TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.bold, height: 1)),
          ...
          const SizedBox(height: 40),
          const Text('SELECCIONA NIVEL', style: TextStyle(color: Colors.white70, fontWeight: FontWeight.bold)),
  ...
  Widget _tutorialHighlight({required bool isActive, required Widget child}) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isActive ? Colors.white : Colors.transparent, width: 4),
      ),
      child: child,
    );
  }
      ...
      children: [
        ...
        Obx(() => controller.sugerencias.isEmpty
          ...
          : Container(
              ...
              child: Column(
                children: controller.sugerencias.map((area) => ListTile(
                  leading: const Icon(Icons.location_searching, color: Colors.orange),
                  title: Text(area.nombre, style: const TextStyle(fontWeight: FontWeight.bold)),
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: sel ? [const BoxShadow(color: Colors.black12, blurRadius: 8)] : null,
            ),
            child: Row(
              children: [
                Icon(Icons.layers, color: sel ? Colors.orange : Colors.white),
                const SizedBox(width: 15),
                Text("PISO $i", style: TextStyle(color: sel ? Colors.orange : Colors.white, fontWeight: FontWeight.bold)),
                if (sel) const Spacer(),
                if (sel) const Icon(Icons.check_circle, color: Colors.orange, size: 18),
              ],
            ),
          ),
        );
  ...
  Widget _buildMissionBanner() {
    return Obx(() {
      ...
      switch (controller.missionStep.value) {
        case 1: msg = "PASO 1: Presiona un botón de 'PISO' en el menú naranja."; icon = Icons.touch_app; break;
        case 2: msg = "PASO 2: Escribe el nombre de una sala en el buscador."; icon = Icons.keyboard; break;
        case 3: msg = "PASO 3: Toca el icono que parpadea en el mapa."; icon = Icons.ads_click; break;
        case 4: msg = "¡LO LOGRASTE! Ya sabes usar el mapa."; icon = Icons.celebration; break;
      }
      return Positioned(
        bottom: 110, left: 40, right: 40,
        child: Container(
          ...
          child: Row(
            children: [
              ...
              Expanded(child: Text(msg, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16))),
              IconButton(icon: const Icon(Icons.close, color: Colors.white54), onPressed: () => controller.cancelarTutorial()),
            ],
          ),
        ),
      );
          ...
          children: categorias.map((cat) => Obx(() {
            ...
            return Padding(
              ...
              child: FilterChip(
                ...
                labelStyle: TextStyle(color: isSelected ? Colors.white : Colors.cyan[900]),
  ...
  Widget _buildFloatingFloorIndicator() => Positioned(
    ...
    child: Container(
      ...
      child: Obx(() => Text("PISO ${controller.pisoActual.value}", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.cyan[900]))),
    ),
  );
  ...
  Widget _buildFloatingMenuButton() {
    return Positioned(
      ...
      left: 30,
      child: Obx(() {
        bool tutorialActivo = controller.missionStep.value > 0 && controller.missionStep.value < 4;
        return Opacity(
          opacity: tutorialActivo ? 0.5 : 1.0,
          child: FloatingActionButton.extended(
            heroTag: 'menu_fab',
            backgroundColor: Colors.white,
            onPressed: tutorialActivo ? null : () => controller.isMenuOpen.toggle(),
            label: Text(controller.isMenuOpen.value ? "CERRAR" : "MENÚ", style: TextStyle(color: tutorialActivo ? Colors.grey : Colors.cyan)),
            icon: Icon(Icons.menu, color: tutorialActivo ? Colors.grey : Colors.cyan),
          ),
        );
      }),
    );
